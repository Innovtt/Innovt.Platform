...
namespace Innovt.Contrib.Authorization.Platform.Application
{
    ...
    {
        private readonly IAuthorizationPolicyRepository authorizationRepository;
        
        //private readonly IAuthorizationRoleRepository authorizationRoleRepository;

        public AuthorizationAppService(IAuthorizationPolicyRepository authorizationRepository)
        {
        ...
        public async Task<Guid> AddPermission(AddPermissionCommand command, CancellationToken cancellationToken)
        {
            ...
            var persistedPermission = await authorizationRepository
                .GetPermissionsBy(command.Scope, command.Resource, cancellationToken: cancellationToken)
        ...
        public async Task RemovePermission(RemovePermissionCommand command, CancellationToken cancellationToken)
        {
            ...
            var permission = await authorizationRepository.GetPermissionsById(command.Id, cancellationToken)
                .ConfigureAwait(false);
        ...
        public async Task<Guid> AddRole(AddRoleCommand command, CancellationToken cancellationToken)
        {
            ...
            var persistedRole = await authorizationRepository.GetRoleByName(command.Name, cancellationToken)
                .ConfigureAwait(false);
        ...
        public async Task<Guid> AddGroup(AddGroupCommand command, CancellationToken cancellationToken)
        {
            ...
            var groupPersisted = await authorizationRepository
                .GetGroupBy(command.Name, cancellationToken).ConfigureAwait(false);
        ...
        public async Task<IList<PermissionDto>> FindPermissionBy(PermissionFilter filter,
            CancellationToken cancellationToken)
        {
            ...
            var a = await authorizationRepository
                .GetPermissionsBy(filter.Scope, filter.Resource, filter.Name, cancellationToken).ConfigureAwait(false);
        ...
        public async Task AddUser(AddUserCommand command, CancellationToken cancellationToken)
        {
            ...
            var user = await authorizationRepository.GetUser(command.Id, cancellationToken).ConfigureAwait(false);
        ...
        public async Task AssignRole(AssignRoleCommand command, CancellationToken cancellationToken)
        {
            ...
            var user = await authorizationRepository.GetUser(command.UserId, cancellationToken).ConfigureAwait(false);
