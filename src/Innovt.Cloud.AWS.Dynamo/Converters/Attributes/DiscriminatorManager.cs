...
public static class DiscriminatorManager
{
    ...
    internal static List<PropertyBuilder> GetDiscriminatorProperties<T>(DynamoContext context, EntityTypeBuilder typeBuilder, 
        ...
        where T : class
    {
        ...
        if (discriminatorValue is null)
            throw new DiscriminatorException($"The instance has no value for discriminator property {discriminatorName}");
        ...
        if(!context.HasTypeBuilder(discriminatorType.Name))
            throw new DiscriminatorException($"The discriminator type {discriminatorType.Name} does not exist in the context.Please check your context.");
