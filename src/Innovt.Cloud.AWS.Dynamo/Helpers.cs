...
internal static class Helpers
{
    ...
    /// If the IndexName is specified in the request, it is set in the IndexName property; otherwise, it uses the primary index.
    ...
    /// <remarks>
    /// This method creates a dictionary of KeysAndAttributes for BatchGetItem based on the provided Table.BatchGetItemRequest.
    /// It processes each item in the request and adds a key-value pair to the result dictionary.
    /// The key is the table name, and the value is a KeysAndAttributes object containing parameters such as ConsistentRead,
    ...
    /// The AttributeConverter.ConvertToAttributeValues method is used to convert the provided attribute values to AttributeValue objects.
    /// </remarks>
    ...
    /// and converts them into a list of strongly typed objects of type T. The conversion is performed by mapping the attribute
    ...
    /// <summary>
    /// Converts a list of dictionaries of AttributeValue objects into two lists of strongly typed objects, one for each specified type (T1 and T2).
    /// </summary>
    ...
    /// <returns>
    /// A tuple containing two lists of strongly typed objects, where the first list (T1) includes objects created by mapping the attribute values
    /// of items that have the specified "EntityType" attribute matching the "splitBy" value to the properties of type T1, and the second list (T2)
    /// includes objects created by mapping the attribute values of other items to the properties of type T2.
    /// </returns>
    /// <remarks>
    /// This method takes a list of dictionaries, where each dictionary represents a set of attribute values for an item,
    /// and converts them into two separate lists of strongly typed objects. The conversion is based on the presence of the "EntityType"
    /// attribute in each dictionary and its value matching the "splitBy" parameter.
    /// Items with the specified "EntityType" attribute value matching "splitBy" will be converted to objects of type T1 and included in the first list.
    ...
    /// <summary>
    /// Converts a list of dictionaries of AttributeValue objects into three lists of strongly typed objects, one for each specified type (T1, T2, and T3).
    /// </summary>
    /// <typeparam name="T1">The desired type for the first list.</typeparam>
    /// <typeparam name="T2">The desired type for the second list.</typeparam>
    /// <typeparam name="T3">The desired type for the third list.</typeparam>
    /// <param name="items">A list of dictionaries, where each dictionary represents a set of attribute values for an item.</param>
    /// <param name="splitBy">An array of string values used to split the items into three lists based on a specific attribute value.</param>
    /// <returns>
    /// A tuple containing three lists of strongly typed objects, where the first list (T1) includes objects created by mapping the attribute values
    /// of items that have the specified "EntityType" attribute matching the first value in the "splitBy" array to the properties of type T1,
    /// the second list (T2) includes objects created by mapping the attribute values of items that have the specified "EntityType" attribute
    /// matching the second value in the "splitBy" array to the properties of type T2, and the third list (T3) includes objects created by mapping
    /// the attribute values of other items to the properties of type T3.
    /// </returns>
    /// <remarks>
    /// This method takes a list of dictionaries, where each dictionary represents a set of attribute values for an item,
    /// and converts them into three separate lists of strongly typed objects. The conversion is based on the presence of the "EntityType"
    /// attribute in each dictionary and its value matching one of the values in the "splitBy" array.
    /// Items with the specified "EntityType" attribute value matching the first value in "splitBy" will be converted to objects of type T1
    /// and included in the first list. Items matching the second value in "splitBy" will be converted to objects of type T2 and included in
    ...
    /// <summary>
    /// Converts a list of dictionaries of AttributeValue objects into four lists of strongly typed objects, one for each specified type (T1, T2, T3, and T4).
    /// </summary>
    /// <typeparam name="T1">The desired type for the first list.</typeparam>
    /// <typeparam name="T2">The desired type for the second list.</typeparam>
    /// <typeparam name="T3">The desired type for the third list.</typeparam>
    ...
    /// <param name="items">A list of dictionaries, where each dictionary represents a set of attribute values for an item.</param>
    /// <param name="splitBy">An array of string values used to split the items into four lists based on a specific attribute value.</param>
    /// <returns>
    /// A tuple containing four lists of strongly typed objects, where the first list (T1) includes objects created by mapping the attribute values
    /// of items that have the specified "EntityType" attribute matching the first value in the "splitBy" array to the properties of type T1,
    /// the second list (T2) includes objects created by mapping the attribute values of items that have the specified "EntityType" attribute
    /// matching the second value in the "splitBy" array to the properties of type T2, the third list (T3) includes objects created by mapping
    /// the attribute values of items that have the specified "EntityType" attribute matching the third value in the "splitBy" array to the properties of type T3,
    /// and the fourth list (T4) includes objects created by mapping the attribute values of other items to the properties of type T4.
    /// </returns>
    /// <remarks>
    /// This method takes a list of dictionaries, where each dictionary represents a set of attribute values for an item,
    /// and converts them into four separate lists of strongly typed objects. The conversion is based on the presence of the "EntityType"
    /// attribute in each dictionary and its value matching one of the values in the "splitBy" array.
    /// Items with the specified "EntityType" attribute value matching the first value in "splitBy" will be converted to objects of type T1
    /// and included in the first list. Items matching the second value in "splitBy" will be converted to objects of type T2 and included in
    /// the second list. Items matching the third value in "splitBy" will be converted to objects of type T3 and included in the third list.
