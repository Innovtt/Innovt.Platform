...
namespace Innovt.Data.EFCore.Repositories
{
    public class RepositoryBase<T> : IRepository<T> where T : class
    {
        ...
        {
            var itens  =  this.FindBy(specification, includes);

            var result = new PagedCollection<T>(itens,specification.Page,specification.PageSize)
            {
            ...
            bool isOrderByDescending = false, Include includes = null)
        {
            ...
            var itens = this.FindBy<TKey>(specification,orderBy,isOrderByDescending, includes);
        ...
        {
          var itens = await this.FindByAsync(specification, includes,cancellationToken);
        ...
        {
            var   itens        = await this.FindByAsync<TKey>(specification,orderBy,isOrderByDescending, includes,cancellationToken);
